...
* Be a Databricks workspace admin in the staging and prod workspaces. Verify that you're an admin by viewing the
  [staging workspace admin console](https://dbc-df0101a0-8706.cloud.databricks.com#setting/accounts) and
  [prod workspace admin console](https://dbc-df0101a0-8706.cloud.databricks.com#setting/accounts). If
...
* Run `aws configure --profile mlopsStackAWS20221108` to configure an AWS CLI profile, passing the access key ID and secret access key from the previous step
* Run `export AWS_PROFILE=mlopsStackAWS20221108` to indicate to Terraform that it should use the newly-created AWS CLI profile
  to authenticate to AWS

### Configure Databricks auth
* Configure a Databricks CLI profile for your staging workspace by running
  ``databricks configure --token --profile "mlopsStackAWS20221108-staging" --host https://dbc-df0101a0-8706.cloud.databricks.com``, 
...
* Create a [Databricks REST API token](https://docs.databricks.com/dev-tools/api/latest/authentication.html#generate-a-personal-access-token)
  in the staging workspace ([link](https://dbc-df0101a0-8706.cloud.databricks.com#setting/account))
  and paste the value into the prompt.
* Configure a Databricks CLI for your prod workspace by running ``databricks configure --token --profile "mlopsStackAWS20221108-prod" --host https://dbc-df0101a0-8706.cloud.databricks.com``
* Create a Databricks REST API token in the prod workspace ([link](https://dbc-df0101a0-8706.cloud.databricks.com#setting/account)).
  and paste the value into the prompt

### Set up service principal user group
Ensure a group named `mlopsStackAWS20221108-service-principals` exists in the staging and prod workspace, e.g.
by checking for the group in the [staging workspace admin console](https://dbc-df0101a0-8706.cloud.databricks.com#setting/accounts/groups) and
[prod workspace admin console](https://dbc-df0101a0-8706.cloud.databricks.com#setting/accounts/groups).
Create the group in staging and/or prod as needed.
Then, grant the `mlopsStackAWS20221108-service-principals` group [token usage permissions](https://docs.databricks.com/administration-guide/access-control/tokens.html#manage-token-permissions-using-the-admin-console)
