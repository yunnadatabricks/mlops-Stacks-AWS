...
resource "aws_s3_bucket" "terraform_state" {
  bucket = "mlopsStackAWS20221108-tfstate"
...
resource "aws_dynamodb_table" "terraform_state_lock" {
  name           = "mlopsStackAWS20221108-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
...
resource "aws_iam_policy" "my-s3-read-policy" {
  name        = "mlopsStackAWS20221108-terraform-state-manage"
  description = "S3 bucket access policy for managing terraform state for mlopsStackAWS20221108"
...
resource "aws_iam_user" "s3_bucket_cicd_user" {
  name = "mlopsStackAWS20221108-terraform-cicd-user"
}
...
resource "aws_s3_bucket" "cicd_terraform_state" {
  bucket = "mlopsStackAWS20221108-cicd-setup-tfstate"
...
resource "aws_dynamodb_table" "cicd_terraform_state_lock" {
  name           = "mlopsStackAWS20221108-cicd-setup-tfstate-lock"
  read_capacity  = 1
  write_capacity = 1
  hash_key       = "LockID"
